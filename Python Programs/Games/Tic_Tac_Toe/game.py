...
class TicTacToe:
    def __init__(self):
        self.board = [' ' for _ in range(9)]  # we will use a list to represent 3x3 board
        self.current_winner = None  # keeps track of winner

    def print_board(self):
        # This is just getting the rows in the board
        for row in [self.board[i*3:(i+1)*3] for i in range(3)]:
            print('| ' + ' | '.join(row) + ' |')

    # 0 | 1 | 2 (number tells us what num corresponds to what box)
    @staticmethod
